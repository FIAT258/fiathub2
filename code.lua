    ...
    staminaConn = RunService.Heartbeat:Connect(function()
        ...
        if char then
            for _,v in pairs(char:GetDescendants()) do
                if v.Name:lower():find("stamina") then
                    pcall(function()
...
local function runAutoGen()
    ...
    local ok, Flow = pcall(function()
        return require(ReplicatedStorage.Modules.Misc.FlowGameManager)
    end)
    if ok and Flow.activeGame then
        Flow.activeGame:EndGame(true)
    end
end
