...
Tabs.Lag:AddParagraph({
    Title = "Lag Bomb (BETA)",
    Content = "Essa função está em beta com script e ela pode te travar junto, tenha certeza!"
})

-- Bomb Count Slider
Options.BombCountSlider = Tabs.Lag:AddSlider("BombCountSlider", {
    Title = "Quantidade de Bombas",
    Description = "Ajuste o número de bombas a serem duplicadas",
    Default = 10,
    Min = 1,
    Max = 1000,
    Rounding = 0,
    ...
    if Value then
        ...
        bombConnection = RunService.Heartbeat:Connect(function()
            if bombCount >= maxBombs then
                ...
                Options.LagBombToggle:SetValue(false)
                return
            end
